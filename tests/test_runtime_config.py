...
class TestRuntimeConfig(unittest.TestCase):
    def test_default_sigmalos2_n_u_gpu_x32(self):
        with mock.patch.object(model_jax_mod, "_prefers_gpu_x32", return_value=True):
            self.assertEqual(model_jax_mod._default_sigmalos2_n_u(), 1024)

    def test_default_sigmalos2_n_u_has_no_env_override(self):
        with mock.patch.dict(model_jax_mod.os.environ, {"JEANSPY_SIGMALOS2_N_U": "777"}, clear=False):
            with mock.patch.object(model_jax_mod, "_prefers_gpu_x32", return_value=True):
                self.assertEqual(model_jax_mod._default_sigmalos2_n_u(), 1024)

    def test_configure_runtime_updates_jax_precision_only(self):
        original = bool(model_jax_mod.jax.config.read("jax_enable_x64"))
        try:
            updated = model_jax_mod.configure_runtime(jax_enable_x64=not original)
            self.assertEqual(updated["jax_enable_x64"], (not original))
        finally:
            model_jax_mod.configure_runtime(jax_enable_x64=original)

    def test_configure_runtime_rejects_legacy_numerical_options(self):
        with self.assertRaises(TypeError):
            model_jax_mod.configure_runtime(sigmalos2_n_u=1024)
    ...
    def test_sigmalos2_jit_matches_eager(self):
        ...
        eager = np.asarray(dsph.sigmalos2(radii, params=params_jax, solver="kernel", jit=False, n_u=256, u_max=2000.0))
        compiled = np.asarray(dsph.sigmalos2(radii, params=params_jax, solver="kernel", jit=True, n_u=256, u_max=2000.0))
