...
def run(output_dir, num_steps=256, resume_steps=128):
    ...
    initial = np.array([np.log10(500.), -1., 0., 0.])
    walkers = initial + rng.normal(size=(16, 4)) * [.05, .05, .05, .5]
    backend = emcee.backends.HDFBackend(str(output_dir / "chain.h5"))
    sampler = emcee.EnsembleSampler(16, 4, log_probability, backend=backend)
    ...
    resumed = emcee.EnsembleSampler(16, 4, log_probability, backend=restored_backend)
