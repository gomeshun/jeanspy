...
class SersicModel(_LegacySersicModel):
    ...
    def __init__(self, *args, deprojection_method: str = "auto", **kwargs):
        ...
        # The historical implementation performs the common Sérsic
        # initialization and loads the bundled coefficient tables.  Give it a
        # method name it already understands, then install the modern choice.
        super().__init__(*args, deprojection_method="numerical", **kwargs)
        self.deprojection_method = deprojection_method

    @staticmethod
    def _eval_vm20_poly(coeff_table, log_x, log_n):
        """Evaluate the VM20-family logarithmic density correction.

        The bundled coefficient tables use the convention

        ``rho_corrected = rho_LGM * 10**P``

        where ``P`` is the bivariate polynomial in ``log10(r/R_e)`` and
        ``log10(n)``.  This sign convention is independently checked against
        numerical Abel inversion in the test suite.
        """
        return _LegacySersicModel._eval_vm20_poly(coeff_table, log_x, log_n)

    def density_3d_VM20(self, r_pc):
        """Vitral & Mamon (2020) 3-D Sérsic density approximation.

        The polynomial correction is applied as

        ``rho_VM20 = rho_LGM * 10**P``.

        It is calibrated for ``0.5 <= n <= 10`` and
        ``1e-3 <= r/R_e <= 1e3``; extrapolation is rejected.
        """
        return super().density_3d_VM20(r_pc)
    ...
    def density_3d_VM20bis(self, r_pc):
        ...
        ``rho_VM20bis = rho_LGM * 10**P`` and is supported for
        ``0.5 <= n <= 3.4`` and ``1e-4 <= r/R_e <= 1e3``.
        """
        return super().density_3d_VM20bis(r_pc)
