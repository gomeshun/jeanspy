...
def main():
    ...
    for short, title in MODULES.items():
        ...
        for name in exports(module):
            ...
            if inspect.isclass(obj):
                methods = [n for n, v in inspect.getmembers(obj)
    ...
    from api_contracts import CONTRACTS, verify_inventory
    verify_inventory(inventory)
    for item in inventory:
        prefix = item["canonical"] + "."
        documented = [path[len(prefix):] for path in CONTRACTS if path.startswith(prefix)]
        item["major_method_contracts"] = sorted(documented)
        for name in documented:
            if name not in item["methods"]:
                raise ValueError(f"Method usage contract does not match API: {prefix}{name}")
